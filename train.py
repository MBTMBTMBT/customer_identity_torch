...
def train(model, optimizer, train_loader, criterion_mask, criterion_pred, epoch, cat_layers, device, mode=0):
    model.train()
    running_loss = 0.0
    ...
    for i, batch in enumerate(progress_bar):
        inputs, mask_labels, _ = batch

        classes = mask_labels.any(dim=-1).any(dim=-1).float()[:,0:cat_layers]
        ...
        pred_loss = criterion_pred(pred_classes, classes)
...
def validate(model, val_loader, criterion_mask, criterion_pred, epoch, cat_layers, device):
    model.eval()
    running_loss = 0.0
    running_accuracy = 0.0
    running_loss = 0.0
...
def test(model, test_loader, criterion_mask, criterion_pred, epoch, cat_layers, device):
    model.eval()
    running_loss = 0.0
    running_accuracy = 0.0
    running_loss = 0.0
...
if __name__ == "__main__":
    ...
    # TensorBoard writer
    writer = SummaryWriter('runs/freeze-half-16')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # train, validate, test
        train_loss, mask_train_loss, pred_train_loss = train(model, optimizer, train_loader, criterion_mask, criterion_pred, epoch, cat_layers, device, mode=mode)
        val_loss, mask_val_loss, pred_val_loss = validate(model, val_loader, criterion_mask, criterion_pred, epoch, cat_layers, device)
        test_loss, mask_test_loss, pred_test_loss = test(model, test_loader, criterion_mask, criterion_pred, epoch, cat_layers, device)
